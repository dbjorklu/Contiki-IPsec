...
struct coap_base_context {
  struct etimer et;
  coap_packet_t subscribe_request[1]; // This way the packet can be treated as pointer as usual. 
  uint8_t request_pending;
  subscription_t subscriptions[2]; // the subscriptionarray is index by the observe token  

  uint8_t **put_char_buf;
//  uint8_t put_char_buf_overflow[MAX_MESSAGE_WITH_CHUNKS-REST_MAX_CHUNK_SIZE];
  uint8_t put_char_buf_overflow[500-REST_MAX_CHUNK_SIZE];
  uint16_t put_char_index;
  struct ActuatorExpression actions[1];
};

static struct coap_base_context ctx;
...
sub1_get(struct jsontree_context *js_ctx)
{
  ...
  if (strncmp(path, "host", 4) == 0) {
    char tmp[36];
    get_ip_str(tmp, 36, &ctx.subscriptions[0].ipaddr);
    jsontree_write_string(js_ctx, tmp); 
  } else if (strncmp(path, "port", 4) == 0) {
    jsontree_write_int(js_ctx, ctx.subscriptions[0].port);
  } else if (strncmp(path, "path", 4) == 0) {
    jsontree_write_string(js_ctx, ctx.subscriptions[0].path);
  } else if (strncmp(path, "token", 4) == 0) {
    char tmp[]="0x00"; sprintf(tmp, "%x", ctx.subscriptions[0].token[0]);
    jsontree_write_string(js_ctx, tmp);
  } else if(strncmp(path, "value", 4) == 0) {
    jsontree_write_string(js_ctx, ctx.subscriptions[0].value);
  } 
  return 0;
}
  ...
  while ((type = jsonparse_next(parser)) != 0) {
    if(type == JSON_TYPE_PAIR_NAME) {
      if(jsonparse_strcmp_value(parser, "host") == 0) {
        ...
        jsonparse_next(parser);
        jsonparse_get_value_as_uip6(parser, &ctx.subscriptions[0].ipaddr);
        update++;
      } else if(jsonparse_strcmp_value(parser, "path") == 0) {
        json_copy_string(parser, ctx.subscriptions[0].path, sizeof(ctx.subscriptions[0].path));
        update++;
      } 
      ...
      // 
        ...
        jsonparse_next(parser);
        jsonparse_next(parser);
        ctx.subscriptions[0].port = jsonparse_get_value_as_int(parser);
        /*if (ctx.subscriptions[0].port == 0) {
          callback_port = 0;
          }*/
        update++;
    }
...
JSONTREE_OBJECT(tree,
                ...
                JSONTREE_PAIR("ctx.subscriptions", &subscriptions_tree));
...
static int
putChar(int a) {
  if (ctx.put_char_index >= MAX_MESSAGE_WITH_CHUNKS) {
    PRINTF("Too big a resource representation, regardless of blockwise\n");
  } else if (ctx.put_char_index >= REST_MAX_CHUNK_SIZE) {
    ...
    ctx.put_char_buf_overflow[ctx.put_char_index++ - REST_MAX_CHUNK_SIZE ] = a;
  }
  else {
    // so far everything fits into one coap message
    (*ctx.put_char_buf)[ctx.put_char_index++] = a;
  }
  return 0;
}
...
void init(struct jsontree_context *json) {
  ...
  if (!initialized) {
    ...
    uip_ip6addr(&(ctx.subscriptions[0].ipaddr), 0xaaaa, 0, 0, 0, 0xc30c, 0x0000, 0x0000, 0x0003); /* cooja2 */
    ctx.subscriptions[0].port = COAP_DEFAULT_PORT;
    strcpy(ctx.subscriptions[0].path, "-");
    strcpy(ctx.subscriptions[0].value, "-");
    ctx.subscriptions[0].token[0] = 1;
    initialized = 1;
  }
}
  ...
  if (get_path(&json, uri_path, len) == 0) {
    ...
    if (*offset > 0) {
      ...
      if (*offset < REST_MAX_CHUNK_SIZE)
        bytes_left = ctx.put_char_index - REST_MAX_CHUNK_SIZE;
      else
        bytes_left = ctx.put_char_index - *offset;

      memcpy(buffer, ctx.put_char_buf_overflow + *offset-preferred_size, MIN(bytes_left, preferred_size));
      ...
      if (REST.get_method_type(request)==METHOD_GET) {
        ctx.put_char_index = 0;
        // ** GET **
        ctx.put_char_buf = &buffer;
        GET(&json);
        printf("ctx.put_char_index %d\n", ctx.put_char_index);
        if (ctx.put_char_index >= preferred_size) {
          *offset += preferred_size;
          printf("setting offset %ld\n", *offset);
        }
      ...
      REST.set_response_payload(response, buffer, MIN(REST_MAX_CHUNK_SIZE, ctx.put_char_index));
...
void execActions() {
  ...
  for (i=0; i < NUM_ACTUATORS; i++) {
    printf("exec action? %d\n", ctx.actions[i].actuator_action != NULL);
    if (ctx.actions[i].actuator_action != NULL) {
      // TODO: parse the expression
      int value = atoi(ctx.subscriptions[0].value) * 15;
      ctx.actions[i].actuator_action(value);
    }
  }
}

void send_observe_request(int token) {
  printf("send observer request token=%d path=%s\n", token, ctx.subscriptions[token-1].path);
  uint8_t token_[TOKEN_LEN] = {token}; 
// prepare request, TID is set by COAP_BLOCKING_REQUEST() 
  ctx.request_pending = token;
  coap_init_message(ctx.subscribe_request, COAP_TYPE_CON, COAP_GET, 0 );
  coap_set_header_uri_path(ctx.subscribe_request, ctx.subscriptions[token-1].path);
  coap_set_header_observe(ctx.subscribe_request, 0xabcd);
  coap_set_header_token(ctx.subscribe_request, token_, TOKEN_LEN);
...
notification_handler(void *response)
{
  ...
  if (len)
  {
    strncpy(ctx.subscriptions[token-1].value, (const char*)chunk, 4);
  }
  // TODO: now just running all actions, should run actions depending on this vlaue

  execActions(atoi(ctx.subscriptions[token-1].value));
}

void coap_base_init() {
  etimer_set(&ctx.et, 1 * CLOCK_SECOND);
